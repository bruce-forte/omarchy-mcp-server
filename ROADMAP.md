...
| F31 | **A person looks for a button.** First live use of the approval prompt on a real desktop: *"there wasn't a button on the notification"*. Correct, and by construction — `omarchy-notification-send` calls `Notify` with an **empty actions array** and rides the click command in an `omarchy-exec-argv` hint, so the whole toast is the click target and no labelled button is available through that CLI at all. F25 recorded the one-action shape; what it did not anticipate is that the *absence of a visible affordance* makes "Click to approve" read as an instruction with no object | The wording was the fault, not the platform. It now says **"Click anywhere on this notification to approve it once"**, which names the target, and — the part that was missing entirely — **"The MCP server panel in the bar also has Always and Deny."** The notification can express *yes* and nothing else; a user who never learns where the other two answers live has no way to stop being asked the same question every time, which is the feature N10 built and never advertised. Pinned by `TestWhatTheNotificationTellsThePerson` |
