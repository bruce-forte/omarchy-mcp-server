...
**Superseded.** A click opens the panel now, and the answer is a labelled
button there — see F31. The rule this paragraph describes is unchanged: silence
still refuses, and only an accept runs anything. What changed is that the
notification is no longer an answer surface at all.
...
**Superseded by N10.** It is on by default now, so the sentence is not needed —
and the claim got stronger rather than weaker. The README paragraph was updated
with it; a headline feature list that still said *"off unless you turn it on"*
would have been the most visible stale claim in the project.
